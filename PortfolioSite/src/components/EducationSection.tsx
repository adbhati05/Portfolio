  ...
  return (
    ...
    >
      <div className="flex flex-col mx-auto max-w-5xl justify-center items-center">
        ...
        <div className="mt-8 w-full grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-6">
          {course.map((course, key) => (
            <div
              key={key}
              className="mt-6 p-4 border border-primary rounded-lg bg-card card-hover" 
            >
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-start md:items-center">
                <div className="text-left">
                  <h3 className="text-lg font-semibold text-left">{course.name}</h3>
                  <p className="text-sm text-primary text-left">{course.topic}</p>
                </div>
                ...
                <span className={`px-4 py-2 rounded-full text-sm font-medium ${
                  ...
                  course.status === "In-Progress" ? "bg-yellow-500/20 text-yellow-500" :
